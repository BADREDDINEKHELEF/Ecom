-- Security hardening migration for StoreDz
-- Run this in Supabase Dashboard → SQL Editor as a single query
-- All statements are idempotent: you can re-run the file safely.
...
-- ============================================================
-- Replace any 16-char truncated hashes with full 64-char SHA-256.
...
-- ============================================================
-- Remove any policy that lets every user see/edit everything.
...
-- ============================================================
-- 5. SAFE EXAMPLE POLICIES (review/adjust to match your columns)
-- ============================================================
-- These assume your tables have `user_id` for auth.uid() matching
-- and `vendor_id` for seller-scoped rows. Adjust if your schema differs.
...
-- Products: a seller sees/manages their own products
...
-- Store settings: scoped to vendor
...
DROP POLICY IF EXISTS store_settings_upsert_own ON public.store_settings;
CREATE POLICY store_settings_upsert_own ON public.store_settings
  FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Customers: sellers see only customers who ordered from them
...
CREATE POLICY customers_vendor_select ON public.customers
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE oi.vendor_id = (SELECT id FROM public.vendors WHERE user_id = auth.uid())
        AND o.phone = customers.phone
    )
  );

-- Orders: sellers see only their own order items' orders
  ...
  FOR SELECT USING (
    EXISTS (
      ...
      WHERE oi.order_id = orders.id
        AND oi.vendor_id = (SELECT id FROM public.vendors WHERE user_id = auth.uid())
    )
  );
...
CREATE POLICY order_items_vendor_select ON public.order_items
  FOR SELECT USING (vendor_id = (SELECT id FROM public.vendors WHERE user_id = auth.uid()));
...
CREATE POLICY order_items_vendor_update ON public.order_items
  FOR UPDATE USING (vendor_id = (SELECT id FROM public.vendors WHERE user_id = auth.uid()));

-- Shipments, messages, payouts, subscriptions: scoped by vendor
...
DROP POLICY IF EXISTS subscriptions_vendor_select ON public.subscriptions;
CREATE POLICY subscriptions_vendor_select ON public.subscriptions
  FOR SELECT USING (vendor_id = (SELECT id FROM public.vendors WHERE user_id = auth.uid()));

-- Reviews & questions: public read, owner/vendor write
...
CREATE POLICY promo_codes_vendor_manage ON public.promo_codes
  FOR ALL USING (vendor_id = (SELECT id FROM public.vendors WHERE user_id = auth.uid()))
  WITH CHECK (vendor_id = (SELECT id FROM public.vendors WHERE user_id = auth.uid()));
...
-- Re-grant necessary functions to authenticated users/service_role
...
-- ============================================================
-- 8. STORAGE BUCKET POLICIES (product/vendor images)
-- ============================================================
-- Replace 'products' and 'vendor-logos' with your actual bucket names.
...
-- Authenticated users can upload to their own vendor folder
