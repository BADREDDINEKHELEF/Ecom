...
const MODULES: Module[] = [
  // ── 1. PAIEMENTS (featured first) ───────────────────────────────────────────
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        steps: [
          { text: 'Téléchargez l\'application "BaridiMob" sur Google Play Store (Android) ou App Store (iPhone). Cherchez "Ø¨Ø±ÙŠØ¯ موب" ou "BaridiMob Algérie".' },
    ...
    lessons: [
      ...
      },
      {
        ...
        steps: [
          ...
          { text: 'Dans Ads Manager : créez une campagne avec l\'objectif "Trafic" ou "Conversions". Ciblez l\'Algérie, tranches d\'?ge 18-45, centres d\'intérêt liés à  votre produit.', tip: 'Commencez avec une audience large (tout l\'Algérie) plutôt que géo-restreinte. Affinez ensuite selon les résultats.' },
  ...
  // ── 7. SERVICE CLIENT ────────────────────────────────────────────────────────
  {
    ...
    desc:    'Transformez chaque client en ambassadeur gr?ce à  une expérience d\'achat mémorable.',
    lessons: [
      {
        id:       'c1',
  ...
  // ── 9. SCALER ───────────────────────────────────────────────────────────────
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        steps: [
          { text: 'Marchés de gros algériens par niche : vêtements → marché Hamiz (Alger), Souk El-Fellah (Oran). Électronique → Bab Ezzouar, Bordj El Kiffan. Cosmétiques → Bir Mourad Ra?s, Belouizdad.', tip: 'Visitez physiquement au moins 3-5 grossistes avant de choisir. Négociez les prix sur le tas.' },
...
function getExtraModules(lang: Lang): Module[] {
  ...
  const pixelsModule: Module = {
    id:      'pixels',
    title:   lang === 'ar' ? 'البيكسل والإعلانات ? MetaØŒ GoogleØŒ TikTok' : lang === 'en' ? 'Pixels & Ads ? Meta, Google, TikTok' : 'Pixels & Publicité ? Meta, Google, TikTok',
    icon:    Target,
    ...
    desc:    lang === 'ar'
      ? 'كيفية Ø¥Ø¹Ø¯Ø§Ø¯ بيكسل Meta ÙˆGoogle Analytics ÙˆTikTok مع الـ API من جهة الخادم Ù„ØªØªØ¨Ø¹ مبيعاتك Ø¨Ø¯Ù‚Ø©.'
      : lang === 'en'
    ...
    lessons: lang === 'ar' ? [
      {
        id: 'px1',
        title: 'Meta Pixel + Conversions API ? Ø§Ù„Ø¥Ø¹Ø¯Ø§Ø¯ الكامل',
        duration: '10 min',
        intro: 'بيكسل Meta ÙŠØªØªØ¨Ø¹ Ø²ÙˆØ§Ø± متجرك في المتصفح. Conversions API ØªØ±Ø³Ù„ بيانات Ø§Ù„Ø´Ø±Ø§Ø¡ مباشرة من خادمنا ? تعمل Ø­ØªÙ‰ مع Ø£Ø¯ÙˆØ§Øª Ø­Ø¬Ø¨ الإعلانات.',
        steps: [
          { text: 'أنشئ Ø­Ø³Ø§Ø¨ Meta Business Suite Ø¹Ù„Ù‰ business.facebook.com Ø¥Ø°Ø§ لم يكن لديك ÙˆØ§Ø­Ø¯. استخدم بريدك الإلكتروني المهني.', tip: 'Ø±Ø¨Ø· حسابك Ø§Ù„Ø´Ø®ØµÙŠ بـ Business Suite يمنحك ÙˆØµÙˆÙ„Ø§Ù‹ لجميع Ø£Ø¯ÙˆØ§Øª الإعلانات.' },
          { text: 'في Business Suite ← Events Manager ← Ø§Ø¶ØºØ· "Ø±Ø¨Ø· مصدر بيانات" ← "Ø§Ù„ÙˆÙŠØ¨" ← "Meta Pixel". أعطِه اسم متجرك وانسخ Pixel ID (Ø³Ù„Ø³Ù„Ø© من 15-16 رقماً).' },
          { text: 'في Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª StoreDz ← قسم "Pixels ÙˆØ§Ù„ØªØªØ¨Ø¹" ← Ø§Ù„ØµÙ‚ Pixel ID في خانة "Meta Pixel ID". احفظ.', tip: 'البيكسل ÙŠØ¨Ø¯Ø£ بالعمل فوراً Ø¨Ø¹Ø¯ الحفظ Ø¹Ù„Ù‰ كل صفحات متجرك.' },
          { text: 'لتفعيل Conversions API: Events Manager ← بيكسلك ← ØªØ¨ÙˆÙŠØ¨ "Ø§Ù„Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª" ← مرر Ø¥Ù„Ù‰ "Conversions API" ← Ø§Ø¶ØºØ· "إنشاء رمز ÙˆØµÙˆÙ„". انسخ الرمز (ÙŠØ¨Ø¯Ø£ بـ EAA...).' },
          { text: 'Ø§Ù„ØµÙ‚ الرمز في خانة "Conversions API Token" في Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª StoreDz. احفظ.' },
          { text: 'Ù„Ù„ØªØ­Ù‚Ù‚: Events Manager ← "Ø§Ø®ØªØ¨Ø§Ø± Ø§Ù„Ø£Ø­Ø¯Ø§Ø«" ← افتح متجرك في ØªØ¨ÙˆÙŠØ¨ Ø¬Ø¯ÙŠØ¯ ← ÙŠØ¬Ø¨ أن ØªØ±Ù‰ PageView Ùˆ ViewContent يظهران Ø®Ù„Ø§Ù„ ثوانٍ.' },
          { text: 'المشتريات ØªØ±Ø³Ù„ مرتين (متصفح + خادم) Ù„Ø¯Ù‚Ø© Ù‚ØµÙˆÙ‰. ØªØ­Ù‚Ù‚ من نتائجك في Ads Manager ← "الأعمدة" ← "المشتريات" Ø¨Ø¹Ø¯ 24-48 Ø³Ø§Ø¹Ø©.' },
        ],
        tip: 'مع تفعيل CAPIØŒ ØªØ±Ù‰ جميع Ø§Ù„ØªØ­ÙˆÙŠÙ„Ø§Øª Ø­ØªÙ‰ Ø¹Ù„Ù‰ iPhone مع Safari Ø§Ù„Ø°ÙŠ ÙŠØ­Ø¬Ø¨ الكوكيز. هذا يحسن نتائج إعلاناتك بشكل كبير.',
        warning: 'Ù„Ø§ تشارك رمز Conversions API مع Ø£ÙŠ Ø´Ø®Øµ. إنه مفتاح Ø³Ø±ÙŠ ? تعامل معه كلمة مرور.',
      },
      {
        id: 'px2',
        title: 'Google Analytics 4 + API Secret ? Ø§Ù„Ø¥Ø¹Ø¯Ø§Ø¯',
        duration: '8 min',
        intro: 'Google Analytics 4 ÙŠØªØ¨Ø¹ سلوك Ø§Ù„Ø²ÙˆØ§Ø± ومسار Ø§Ù„Ø´Ø±Ø§Ø¡. مع Measurement Protocol API ÙŠØ±Ø³Ù„ StoreDz بيانات Ø§Ù„Ø´Ø±Ø§Ø¡ مباشرة من الخادم.',
        steps: [
          { text: 'في analytics.google.com ← Ø§Ø¶ØºØ· "إنشاء" ← "Ø®Ø§ØµÙŠØ©" ← Ø£Ø¯Ø®Ù„ اسم متجرك ← Ø§Ø®ØªØ± المنطقة الزمنية (Ø§Ù„Ø¬Ø²Ø§Ø¦Ø± = Africa/Algiers) ← أنهِ إنشاء Ø§Ù„Ø®Ø§ØµÙŠØ©.' },
          { text: 'Ø§Ø¨Ø­Ø« عن Measurement ID: Ø§Ù„Ø®Ø§ØµÙŠØ© Ø§Ù„Ø¬Ø¯ÙŠØ¯Ø© ← Admin ← Data Streams ← Ø§Ø¶ØºØ· Ø¹Ù„Ù‰ فلوك ← انسخ Measurement ID (ÙŠØ¨Ø¯Ø£ بـ G-).' },
          { text: 'في Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª StoreDz ← قسم Google Analytics 4 ← Ø§Ù„ØµÙ‚ Measurement ID في خانة "Measurement ID". احفظ.', tip: 'هذا يضيف الـ gtag.js ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹ Ø¹Ù„Ù‰ كل صفحات متجرك.' },
          { text: 'لـ API Secret: نفس صفحة Data Stream ← "Measurement Protocol API secrets" ← "Create" ← أعطه اسماً ← انسخ القيمة.' },
          { text: 'Ø§Ù„ØµÙ‚ API Secret في خانة "API Secret (Measurement Protocol)" في Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª StoreDz. احفظ.' },
          { text: 'Ù„Ù„ØªØ­Ù‚Ù‚: analytics.google.com ← Reports ← Realtime ← افتح متجرك في ØªØ¨ÙˆÙŠØ¨ Ø¢Ø®Ø± ← ÙŠØ¬Ø¨ أن ØªØ±Ù‰ مستخدم نشط.' },
        ],
        tip: 'GA4 مجاني تماماً. Ø¨Ø¹Ø¯ 24-48 Ø³Ø§Ø¹Ø©ØŒ Ø³ØªØ¬Ø¯ ØªÙ‚Ø§Ø±ÙŠØ± تفصيلية عن مصادر Ø§Ù„Ø²ÙˆØ§Ø±ØŒ الصفحات الأكثر مشاهدة والمنتجات الأكثر مبيعاً.',
      },
      {
        id: 'px3',
        title: 'TikTok Pixel + Events API ? Ø§Ù„ØªØ«Ø¨ÙŠØª Ø®Ø·ÙˆØ© Ø¨Ø®Ø·ÙˆØ©',
        duration: '8 min',
        intro: 'TikTok Pixel ÙŠÙ‚ÙŠØ³ Ø£Ø¯Ø§Ø¡ إعلاناتك Ø¹Ù„Ù‰ TikTok. Events API ÙŠØ±Ø³Ù„ بيانات Ø§Ù„Ø´Ø±Ø§Ø¡ من خادمنا مباشرة لتحسين Ø¹Ø§Ø¦Ø¯ إعلاناتك.',
        steps: [
          { text: 'Ø§Ø¯Ø®Ù„ TikTok Ads Manager Ø¹Ù„Ù‰ ads.tiktok.com ← Assets ← Events ← "Web Events" ← "Create Pixel". أعطه اسم متجرك.', tip: 'ØªØ­ØªØ§Ø¬ Ø­Ø³Ø§Ø¨ TikTok for Business منفصل عن Ø§Ù„Ø­Ø³Ø§Ø¨ Ø§Ù„Ø´Ø®ØµÙŠ.' },
          { text: 'Ø§Ø®ØªØ± "TikTok Pixel" ← Ø§ØªØ¨Ø¹ Ø®Ø·ÙˆØ§Øª الإنشاء ← في نهاية Ø§Ù„ØªØ«Ø¨ÙŠØª انسخ Pixel ID (ÙŠØ¨Ø¯Ø£ بـ C...).' },
          { text: 'في Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª StoreDz ← قسم TikTok ← Ø§Ù„ØµÙ‚ Pixel ID في خانة "TikTok Pixel ID". احفظ.' },
          { text: 'Ù„Ù„Ø­ØµÙˆÙ„ Ø¹Ù„Ù‰ Events API Token: Events Manager ← بيكسلك ← "API Access Token" ← Ø§Ø¶ØºØ· "Generate". انسخ الرمز.' },
          { text: 'Ø§Ù„ØµÙ‚ الرمز في "Events API Access Token" في Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª StoreDz. احفظ.' },
          { text: 'Ù„Ù„ØªØ­Ù‚Ù‚: Assets ← Events ← بيكسلك ← "Test Events" ← Ø£Ø¯Ø®Ù„ Ø±Ø§Ø¨Ø· متجرك ÙˆØ§Ø¶ØºØ· "Test". ÙŠØ¬Ø¨ أن ØªØ±Ù‰ Ø§Ù„Ø£Ø­Ø¯Ø§Ø« تظهر.', tip: 'TikTok ÙŠØ£Ø®Ø° Ø¹Ø§Ø¯Ø©Ù‹ 2-4 Ø³Ø§Ø¹Ø§Øª Ù„Ø¨Ø¯Ø¡ Ø¹Ø±Ø¶ بيانات Ø§Ù„Ø£Ø­Ø¯Ø§Ø« في Ø§Ù„ÙˆÙ‚Øª الفعلي.' },
        ],
        tip: 'إعلانات TikTok تعمل بشكل ممتاز للمنتجات المرئية (ملابس، أكسسوارات، منتجات منزلية). البيكسل + Events API يحسن استهداف الجمهور ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹.',
        warning: 'تأكد من موافقة Ø³ÙŠØ§Ø³Ø© إعلانات TikTok Ø¹Ù„Ù‰ منتجك Ù‚Ø¨Ù„ إنشاء الحملة. Ø¨Ø¹Ø¶ فئات المنتجات لها Ù‚ÙŠÙˆØ¯ Ø®Ø§ØµØ©.',
      },
    ] : lang === 'en' ? [
      {
        id: 'px1',
  ...
  const deliveryModule: Module = {
    id:      'delivery-api',
    title:   lang === 'ar' ? 'API شركات Ø§Ù„ØªÙˆØµÙŠÙ„ ? YalidineØŒ ZR ExpressØŒ Procolis' : lang === 'en' ? 'Delivery APIs ? Yalidine, ZR Express, Procolis' : 'API Transporteurs ? Yalidine, ZR Express, Procolis',
    icon:    Link2,
    color:   'text-teal-600',
    bg:      'bg-teal-50',
    badge:   lang === 'ar' ? 'متقدم' : lang === 'en' ? 'Advanced' : 'Avancé',
    ...
    desc:    lang === 'ar'
      ? 'Ø§Ø±Ø¨Ø· متجرك بـ API شركات Ø§Ù„ØªÙˆØµÙŠÙ„ لإنشاء Ø¨ÙˆÙ„ÙŠØµØ§Øª الشحن بنقرة ÙˆØ§Ø­Ø¯Ø© ÙˆØªØªØ¨Ø¹ Ø§Ù„Ø·Ø±ÙˆØ¯ ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹.'
      : lang === 'en'
    ...
    lessons: lang === 'ar' ? [
      {
        ...
        duration: '5 min',
        intro: 'الـ API (واجهة برمجة Ø§Ù„ØªØ·Ø¨ÙŠÙ‚Ø§Øª) ÙŠØ±Ø¨Ø· StoreDz مباشرة بنظام شركة Ø§Ù„ØªÙˆØµÙŠÙ„. Ø¨Ø¯Ù„Ø§Ù‹ من إنشاء كل Ø¨ÙˆÙ„ÙŠØµØ© ÙŠØ¯ÙˆÙŠØ§Ù‹ Ø¹Ù„Ù‰ موقع الشركة، يمكنك إنشاؤها بنقرة ÙˆØ§Ø­Ø¯Ø© من Ù„ÙˆØ­Ø© التحكم.',
        steps: [
          { text: 'بدون API: تفتح موقع شركة Ø§Ù„ØªÙˆØµÙŠÙ„ØŒ تملأ بيانات الزبون ÙŠØ¯ÙˆÙŠØ§Ù‹ØŒ ØªØ·Ø¨Ø¹ Ø§Ù„Ø¨ÙˆÙ„ÙŠØµØ©. كل Ø·Ø±Ø¯ ÙŠØ£Ø®Ø° 3-5 Ø¯Ù‚Ø§Ø¦Ù‚.', tip: 'بدون API مقبول لـ 1-10 Ø·Ø±ÙˆØ¯ يومياً. فوق ذلك، Ø§Ù„ÙˆÙ‚Øª المهدر كبير Ø¬Ø¯Ø§Ù‹.' },
          { text: 'مع API: ØªØ¶ØºØ· "إنشاء Ø¨ÙˆÙ„ÙŠØµØ©" في StoreDz ← البيانات تنتقل ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹ ← ØªØ·Ø¨Ø¹ Ø§Ù„Ø¨ÙˆÙ„ÙŠØµØ© مباشرة. كل Ø·Ø±Ø¯ ÙŠØ£Ø®Ø° 20 ثانية.' },
          { text: 'المتطلبات: Ø­Ø³Ø§Ø¨ مهني (ØªØ¬Ø§Ø±ÙŠ) مفعل عند شركة Ø§Ù„ØªÙˆØµÙŠÙ„. Ø§Ù„Ø­Ø³Ø§Ø¨Ø§Øª Ø§Ù„Ø´Ø®ØµÙŠØ© Ø¹Ø§Ø¯Ø©Ù‹ Ù„Ø§ تمنح ÙˆØµÙˆÙ„ API.', tip: 'افتح حسابك Ø§Ù„ØªØ¬Ø§Ø±ÙŠ مبكراً ? التفعيل Ù‚Ø¯ ÙŠØ£Ø®Ø° 1-3 أيام عمل.' },
          { text: 'Ø¨Ø¹Ø¯ تفعيل حسابك Ø§Ù„ØªØ¬Ø§Ø±ÙŠØŒ Ø§Ø¨Ø­Ø« في Ù„ÙˆØ­Ø© التحكم Ø§Ù„Ø®Ø§ØµØ© بك عن قسم "API" Ø£Ùˆ "Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª المطور" Ù„Ù„Ø­ØµÙˆÙ„ Ø¹Ù„Ù‰ مفاتيح Ø§Ù„ÙˆØµÙˆÙ„.' },
          { text: 'في StoreDz: Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª ← "Ø§Ù„ØªÙˆØµÙŠÙ„ والـ API" ← Ø§Ø®ØªØ± شركة Ø§Ù„ØªÙˆØµÙŠÙ„ ← Ø£Ø¯Ø®Ù„ المفاتيح ← احفظ ÙˆØ§Ø®ØªØ¨Ø± Ø§Ù„Ø§ØªØµØ§Ù„.' },
        ],
        tip: 'Ø§Ø¨Ø¯Ø£ بشركة ØªÙˆØµÙŠÙ„ ÙˆØ§Ø­Ø¯Ø© فقط. أتقن الـ API معها Ù‚Ø¨Ù„ إضافة Ø£Ø®Ø±Ù‰. Yalidine مُنصح به للمبتدئين Ù„ØªÙˆØ«ÙŠÙ‚ API Ø§Ù„Ø¬ÙŠØ¯.',
      },
      {
        id: 'del2',
        title: 'Yalidine API ? إنشاء Ø§Ù„Ø­Ø³Ø§Ø¨ ÙˆØ§Ù„Ø­ØµÙˆÙ„ Ø¹Ù„Ù‰ المفاتيح',
        duration: '8 min',
        intro: 'Yalidine هو أشهر شركة ØªÙˆØµÙŠÙ„ في Ø§Ù„Ø¬Ø²Ø§Ø¦Ø± Ù„Ù„ØªØ¬Ø§Ø±Ø© الإلكترونية. API Ø¬ÙŠØ¯ Ø§Ù„ØªÙˆØ«ÙŠÙ‚ وسهل الاستخدام مع ØªØºØ·ÙŠØ© 58 ÙˆÙ„Ø§ÙŠØ©.',
        steps: [
          { text: 'اذهب Ø¥Ù„Ù‰ yalidine.app ← "إنشاء Ø­Ø³Ø§Ø¨" ← Ø§Ø®ØªØ± "Ø­Ø³Ø§Ø¨ ØªØ¬Ø§Ø±ÙŠ". امتلاء نموذج Ø§Ù„ØªØ³Ø¬ÙŠÙ„ كاملاً ÙŠØ³Ø±Ø¹ التفعيل.', tip: 'أضف رقم Ø§Ù„Ø³Ø¬Ù„ Ø§Ù„ØªØ¬Ø§Ø±ÙŠ Ø¥Ø°Ø§ توفر لديك ? ÙŠØ³Ø§Ø¹Ø¯ في التفعيل Ø§Ù„Ø³Ø±ÙŠØ¹.' },
          { text: 'Ø¨Ø¹Ø¯ مراجعة وتفعيل حسابك (1-3 أيام عمل)ØŒ Ø³Ø¬Ù„ Ø§Ù„Ø¯Ø®ÙˆÙ„ ← انتقل Ø¥Ù„Ù‰ "Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø§Ù„Ø­Ø³Ø§Ø¨" ← قسم "API".' },
          { text: 'Ø³ØªØ¬Ø¯: Centre ID (رقم مركزك) ÙˆToken (رمز Ø§Ù„ÙˆØµÙˆÙ„). انسخهما بعناية ? هذان هما مفتاحا Ø§Ù„Ø§ØªØµØ§Ù„.', tip: 'احفظ Token في مكان آمن. Ø¥Ø°Ø§ فقدته يمكنك إنشاء رمز Ø¬Ø¯ÙŠØ¯ لكن الرمز القديم سيُلغى.' },
          { text: 'في StoreDz: Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª ← "Ø§Ù„ØªÙˆØµÙŠÙ„ والـ API" ← Yalidine ← Ø£Ø¯Ø®Ù„ Centre ID ÙˆToken. Ø§Ø¶ØºØ· "Ø§Ø®ØªØ¨Ø§Ø± Ø§Ù„Ø§ØªØµØ§Ù„".' },
          { text: 'Ø§Ø®ØªØ¨Ø§Ø± ناجح ← فعّل "الإنشاء Ø§Ù„ØªÙ„Ù‚Ø§Ø¦ÙŠ Ù„Ù„Ø¨ÙˆÙ„ÙŠØµØ©". الآن عند تأكيد Ø£ÙŠ Ø·Ù„Ø¨ØŒ Ø§Ù„Ø¨ÙˆÙ„ÙŠØµØ© تُنشأ ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹.', tip: 'Ø£ÙˆÙ„Ø§Ù‹ Ø§Ø®ØªØ¨Ø± Ø¨Ø·Ù„Ø¨ Ø­Ù‚ÙŠÙ‚ÙŠ منخفض القيمة للتأكد من ØµØ­Ø© بيانات الزبون Ù‚Ø¨Ù„ الاعتماد الكامل Ø¹Ù„Ù‰ الـ API.' },
          { text: 'Ù„Ø·Ø¨Ø§Ø¹Ø© Ø§Ù„Ø¨ÙˆÙ„ÙŠØµØ©: Ù„ÙˆØ­Ø© التحكم ← Ø§Ù„Ø·Ù„Ø¨ ← "Ø·Ø¨Ø§Ø¹Ø© Ø¨ÙˆÙ„ÙŠØµØ©". الملصق يُطبع بحجم A6 Ø£Ùˆ A4 مقسم.' },
        ],
        tip: 'Yalidine يوفر webhook Ù„Ù„Ø¥Ø´Ø¹Ø§Ø±Ø§Øª الفورية عند ØªØºÙŠÙŠØ± Ø­Ø§Ù„Ø© Ø§Ù„Ø·Ø±Ø¯. فعّله في Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª API Ù„ØªØ­Ø¯ÙŠØ« Ø­Ø§Ù„Ø§Øª Ø§Ù„Ø·Ù„Ø¨Ø§Øª ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹ في StoreDz.',
        warning: 'تأكد من ØµØ­Ø© عنوان الزبون Ù‚Ø¨Ù„ إنشاء Ø§Ù„Ø¨ÙˆÙ„ÙŠØµØ©. Ø§Ù„ØªØ¹Ø¯ÙŠÙ„ Ø¨Ø¹Ø¯ الإنشاء ÙŠØªØ·Ù„Ø¨ Ø§Ù„Ø§ØªØµØ§Ù„ بخدمة العملاء.',
      },
      {
        id: 'del3',
        title: 'ZR Express API ? Ø³Ø±Ø¹Ø© في المدن الكبرى',
        duration: '8 min',
        intro: 'ZR Express Ø³Ø±ÙŠØ¹ Ø¬Ø¯Ø§Ù‹ في المدن الكبرى (Ø§Ù„Ø¬Ø²Ø§Ø¦Ø±ØŒ وهران، قسنطينة ? ØªÙˆØµÙŠÙ„ J+1). مثالي Ø¥Ø°Ø§ كان معظم زبائنك في الشمال.',
        steps: [
          { text: 'اذهب Ø¥Ù„Ù‰ zrexpress.dz ← "اشتراك" ← "Ø­Ø³Ø§Ø¨ مهني". Ø³ØªØªÙ„Ù‚Ù‰ تأكيداً Ø¨Ø§Ù„Ø¨Ø±ÙŠØ¯ الإلكتروني.' },
          { text: 'Ø¨Ø¹Ø¯ التفعيل، Ø³Ø¬Ù„ Ø§Ù„Ø¯Ø®ÙˆÙ„ ← "Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª" Ø£Ùˆ "Mon compte" ← Ø§Ø¨Ø­Ø« عن مفاتيح API Ø£Ùˆ "Intégration".' },
          { text: 'انسخ: Client ID ÙˆAPI Key (Ø£Ùˆ Token). Ø¨Ø¹Ø¶ Ø§Ù„Ø­Ø³Ø§Ø¨Ø§Øª تستخدم نظام اسم المستخدم وكلمة المرور Ø¨Ø¯Ù„Ø§Ù‹ من التوكن.', tip: 'Ø¥Ø°Ø§ لم ØªØ¬Ø¯ مفاتيح API مباشرة، ØªÙˆØ§ØµÙ„ مع فريق ZR Express Ø¹Ø¨Ø± Ø§Ù„ÙˆØ§ØªØ³Ø§Ø¨ Ø£Ùˆ Ø§Ù„Ø¨Ø±ÙŠØ¯ ? يرسلونها Ø®Ù„Ø§Ù„ 24 Ø³Ø§Ø¹Ø©.' },
          { text: 'في StoreDz: Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª ← "Ø§Ù„ØªÙˆØµÙŠÙ„ والـ API" ← ZR Express ← Ø£Ø¯Ø®Ù„ المعرفات. Ø§Ø®ØªØ¨Ø± Ø§Ù„Ø§ØªØµØ§Ù„.' },
          { text: 'فعّل ZR Express كـ "ناقل ثانوي". يمكنك توجيه Ø§Ù„Ø·Ù„Ø¨Ø§Øª ÙŠØ¯ÙˆÙŠØ§Ù‹ بين Yalidine ÙˆZR Express Ø­Ø³Ø¨ Ø§Ù„ÙˆÙ„Ø§ÙŠØ© ÙˆØ§Ù„Ø£ÙˆÙ„ÙˆÙŠØ©.' },
          { text: 'للمدن الكبرى، استخدم ZR Express Ù„Ù„Ø³Ø±Ø¹Ø©. للمناطق النائية، استخدم Yalidine Ù„Ù„ØªØºØ·ÙŠØ©.' },
        ],
        tip: 'ZR Express يتميز بمرونة Ø£ÙˆÙ‚Ø§Øª Ø§Ù„ØªØ­ØµÙŠÙ„ ? يمكنك Ø¬Ø¯ÙˆÙ„Ø© Ø§Ù„ØªØ­ØµÙŠÙ„ لنفس اليوم في معظم المدن الكبرى.',
      },
      {
        id: 'del4',
        title: 'Procolis ? Ø£Ø¬Ø±ÙŠØºØ§ØªÙˆØ± ÙŠØ®ØªØ§Ø± Ø§Ù„Ø£Ø±Ø®Øµ ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹',
        duration: '7 min',
        intro: 'Procolis Ù„ÙŠØ³ شركة ØªÙˆØµÙŠÙ„ ? هو منصة تجمع Ø¹Ø¯Ø© شركات (YalidineØŒ ZR ExpressØŒ وغيرها) ÙˆØªØ®ØªØ§Ø± Ø§Ù„Ø£Ø±Ø®Øµ ÙˆØ§Ù„Ø£Ø³Ø±Ø¹ لكل ÙˆÙ„Ø§ÙŠØ© ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹.',
        steps: [
          { text: 'اذهب Ø¥Ù„Ù‰ procolis.com ← "Créer un compte" ← Ø£Ø¯Ø®Ù„ بياناتك Ø§Ù„ØªØ¬Ø§Ø±ÙŠØ©. التفعيل Ø¹Ø§Ø¯Ø©Ù‹ Ø®Ù„Ø§Ù„ 24-48 Ø³Ø§Ø¹Ø©.', tip: 'Procolis ÙŠØ·Ù„Ø¨ Ø¹Ù‚Ø¯ ØªØ¬Ø§Ø±ÙŠ Ø¨Ø³ÙŠØ·. اقرأه Ø¬ÙŠØ¯Ø§Ù‹ ? ÙŠÙˆØ¶Ø­ Ø§Ù„Ø£Ø³Ø¹Ø§Ø± ÙˆØ´Ø±ÙˆØ· Ø§Ù„ØªØ­ØµÙŠÙ„.' },
          { text: 'Ø¨Ø¹Ø¯ التفعيل: Dashboard ← "Paramètres API" Ø£Ùˆ "Intégration" ← Ø§Ø­ØµÙ„ Ø¹Ù„Ù‰ API Key Ø§Ù„Ø®Ø§Øµ بك.' },
          { text: 'Ø±Ø¨Ø· شركات Ø§Ù„ØªÙˆØµÙŠÙ„: في Ù„ÙˆØ­Ø© ProcolisØŒ أضف حساباتك عند Yalidine ÙˆZR Express (Ø¥Ø°Ø§ كانت لديك). Procolis ÙŠØ±Ø¨Ø· بينها ويوجه Ø§Ù„Ø·Ù„Ø¨Ø§Øª ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹.' },
          { text: 'في StoreDz: Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª ← "Ø§Ù„ØªÙˆØµÙŠÙ„ والـ API" ← Procolis ← Ø£Ø¯Ø®Ù„ API Key. Ø§Ø®ØªØ¨Ø± Ø§Ù„Ø§ØªØµØ§Ù„.' },
          { text: 'عند إنشاء Ø¨ÙˆÙ„ÙŠØµØ©ØŒ Procolis يقارن Ø£Ø³Ø¹Ø§Ø± جميع الشركات لتلك Ø§Ù„ÙˆÙ„Ø§ÙŠØ© ÙˆÙŠØ®ØªØ§Ø± الأنسب. توفير 15-25% في التكاليف ممكن عند الحجم Ø§Ù„Ø¹Ø§Ù„ÙŠ.', tip: 'فعّل "Ø§Ø®ØªÙŠØ§Ø± ØªÙ„Ù‚Ø§Ø¦ÙŠ للناقل" في Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Procolis. يمكنك ÙˆØ¶Ø¹ Ø£ÙˆÙ„ÙˆÙŠØ© (Ø³Ø¹Ø± أم Ø³Ø±Ø¹Ø©) لكل منطقة.' },
          { text: 'متابعة كل Ø§Ù„Ø·Ø±ÙˆØ¯ من مكان ÙˆØ§Ø­Ø¯: Procolis Dashboard يجمع ØªÙ‚Ø§Ø±ÙŠØ± جميع الشركات في واجهة موحدة.' },
        ],
        tip: 'Procolis مثالي عند Ø§Ù„ÙˆØµÙˆÙ„ لـ 50+ Ø·Ø±Ø¯/شهر. عند هذا الحجم، الفارق في التكلفة ÙŠØµØ¨Ø­ كبيراً ÙˆØ¥Ø¯Ø§Ø±Ø© شركة ÙˆØ§Ø­Ø¯Ø© أسهل بكثير.',
        warning: 'Procolis يضيف Ø·Ø¨Ù‚Ø© ÙˆØ³ÙŠØ·Ø©. في Ø­Ø§Ù„Ø© مشكلة مع Ø·Ø±Ø¯ØŒ Ø§Ù„ØªÙˆØ§ØµÙ„ مع Procolis Ø£ÙˆÙ„Ø§Ù‹ ثم هم يتابعون مع شركة Ø§Ù„ØªÙˆØµÙŠÙ„.',
      },
    ] : lang === 'en' ? [
      {
        id: 'del1',
    ...
    ] : [
      {
        ...
        tip: 'Commencez avec un seul transporteur. Maîtrisez son API avant d\'en ajouter un autre. Yalidine est recommandé pour les débutants gr?ce à  une documentation API bien faite.',
      },
      {
        id: 'del2',
