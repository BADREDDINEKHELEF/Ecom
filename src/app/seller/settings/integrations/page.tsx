    ...
    try {
      ...
      setTestResults((prev) => ({
        ...
        [`${integrationName}-${action}`]: {
          ...
          raw: data.raw || data,
          message: data.message || data.error || (res.ok ? 'VÃƒÂ©rification rÃƒÂ©ussie' : 'Ãƒâ€°chec de la vÃƒÂ©rification'),
          rate: data.rate || null,
        },
      }))
      loadHealth()
      ...
      case 'connected':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-green-50 text-green-700 border border-green-200">
            ...
            {isRTL ? 'Ã™â€¦Ã˜ÂªÃ˜ÂµÃ™â€ž' : 'ConnectÃƒÂ©'}
          </span>
        )
      case 'failed':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-red-50 text-red-700 border border-red-200">
            ...
            {isRTL ? 'Ã™ÂÃ˜Â´Ã™â€ž Ã˜Â§Ã™â€žÃ˜Â§Ã˜ÂªÃ˜ÂµÃ˜Â§Ã™â€ž' : 'Ãƒâ€°chec d\'authentification'}
          </span>
        )
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-gray-50 text-gray-500 border border-gray-200">
            ...
            {isRTL ? 'Ã˜Â¨Ã˜Â­Ã˜Â§Ã˜Â¬Ã˜Â© Ã˜Â¥Ã™â€žÃ™â€° Ã˜ÂªÃ™â€¡Ã™Å Ã˜Â¦Ã˜Â©' : 'Configuration requise'}
          </span>
        )
    }
  }
        ...
        <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2">
              <Gauge className="w-6 h-6 text-indigo-600" /> {isRTL ? 'Ã˜Â§Ã™â€žÃ˜ÂªÃ˜Â­Ã™â€šÃ™â€š Ã™â€¦Ã™â€  Ã˜Â§Ã™â€žÃ˜Â¥Ã˜Â¹Ã˜Â¯Ã˜Â§Ã˜Â¯Ã˜Â§Ã˜Âª' : 'VÃƒÂ©rification des IntÃƒÂ©grations'}
            </h1>
            <p className="text-gray-500 text-sm mt-1">{isRTL ? 'Ã˜ÂªÃ˜Â­Ã™â€šÃ™â€š Ã™â€¦Ã™â€  Ã˜Â±Ã˜Â¨Ã˜Â· Ã˜Â´Ã˜Â±Ã™Æ’Ã˜Â§Ã˜Âª Ã˜Â§Ã™â€žÃ˜Â´Ã˜Â­Ã™â€  Ã™Ë†Ã˜Â§Ã™â€žÃ˜Â¨Ã™Æ’Ã˜Â³Ã™â€ž Ã™ÂÃ™Å  Ã˜Â§Ã™â€žÃ™Ë†Ã™â€šÃ˜Âª Ã˜Â§Ã™â€žÃ™ÂÃ˜Â¹Ã™â€žÃ™Å ' : 'Testez et validez vos API de livraison et tracking publicitaire en temps rÃƒÂ©el.'}</p>
          </div>
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            >
              ...
              <span>{isRTL ? 'Ã™Ë†Ã˜Â¶Ã˜Â¹ Ã˜Â§Ã™â€žÃ™â€¦Ã˜Â·Ã™Ë†Ã˜Â±' : 'Mode DÃƒÂ©veloppeur'}</span>
            </button>
            <button
              onClick={loadHealth}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-bold uppercase">{isRTL ? 'Ã™â€ Ã˜Â´Ã˜Â·' : 'ConnectÃƒÂ©s'}</p>
              ...
              <div className="flex items-center gap-3 border-t sm:border-t-0 sm:border-l border-gray-100 sm:pl-4">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-bold uppercase">{isRTL ? 'Ã™ÂÃ˜Â´Ã™â€ž Ã˜Â§Ã™â€žÃ˜Â±Ã˜Â¨Ã˜Â·' : 'Erreurs Authentification'}</p>
                  <p className="text-xl font-black text-gray-900">
              ...
              <div className="flex items-center gap-3 border-t sm:border-t-0 sm:border-l border-gray-100 sm:pl-4">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-bold uppercase">{isRTL ? 'Ã™â€žÃ™â€¦ Ã˜ÂªÃ˜ÂªÃ™â€¦ Ã˜ÂªÃ™â€¡Ã™Å Ã˜Â¦Ã˜ÂªÃ™â€¡' : 'Non ConfigurÃƒÂ©s'}</p>
                  <p className="text-xl font-black text-gray-900">
                ...
                return (
                  <div key={integration.name} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
                      ...
                      <div className="flex items-center gap-2">
                        {integration.type === 'delivery' ? (
                          ...
                          >
                            ...
                            <span>{isRTL ? 'Ã™ÂÃ˜Â­Ã˜Âµ Ã˜Â§Ã™â€žÃ˜Â§Ã˜ÂªÃ˜ÂµÃ˜Â§Ã™â€ž' : 'Tester la Connexion'}</span>
                          </button>
                        ) : (
                          <button
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-xs bg-gray-50/50 rounded-xl p-4 border border-gray-100">
                      <div>
                        <p className="text-gray-400 font-bold uppercase">{isRTL ? 'Ã˜Â¢Ã˜Â®Ã˜Â± Ã™â€ Ã˜Â¬Ã˜Â§Ã˜Â­' : 'Dernier succÃƒÂ¨s'}</p>
                        <p className="text-gray-700 font-semibold mt-0.5">
                          {health?.last_success_at ? new Date(health.last_success_at).toLocaleString() : 'Ã¢â‚¬â€'}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-400 font-bold uppercase">{isRTL ? 'Ã˜Â¢Ã˜Â®Ã˜Â± Ã™ÂÃ˜Â´Ã™â€ž' : 'Dernier ÃƒÂ©chec'}</p>
                        <p className="text-gray-700 font-semibold mt-0.5">
                          {health?.last_failure_at ? new Date(health.last_failure_at).toLocaleString() : 'Ã¢â‚¬â€'}
                        </p>
                      </div>
                      <div className="sm:col-span-2 md:col-span-1">
                        <p className="text-gray-400 font-bold uppercase">{isRTL ? 'Ã˜Â¢Ã˜Â®Ã˜Â± Ã˜Â®Ã˜Â·Ã˜Â£' : 'Dernier message d\'erreur'}</p>
                    ...
                    {integration.type === 'delivery' && integration.name !== 'yassir' && (
                      <div className="border-t border-gray-100 pt-4 space-y-3">
                        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">{isRTL ? 'Ã˜ÂªÃ˜Â¬Ã˜Â±Ã˜Â¨Ã˜Â© Ã˜Â­Ã˜Â³Ã˜Â§Ã˜Â¨ Ã˜Â§Ã™â€žÃ˜ÂªÃ˜Â³Ã˜Â¹Ã™Å Ã˜Â±Ã˜Â©' : 'Test de Tarification Temps RÃƒÂ©el'}</p>
                        <div className="flex flex-col sm:flex-row gap-3">
                          ...
                          >
                            ...
                            <span>{isRTL ? 'Ã˜Â­Ã˜Â³Ã˜Â§Ã˜Â¨ Ã˜Â§Ã™â€žÃ˜Â³Ã˜Â¹Ã˜Â±' : 'Calculer le tarif'}</span>
                          </button>
                        </div>
                        ...
                        {quoteResult && (
                          <div className={`rounded-xl p-3 border text-xs ${quoteResult.ok ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
                            {quoteResult.ok ? (
                              <div>
                                <span className="font-black">Tarif rÃƒÂ©cupÃƒÂ©rÃƒÂ©: </span>
                              ...
                              <p className="font-semibold">{quoteResult.message || 'Impossible de rÃƒÂ©cupÃƒÂ©rer la tarification'}</p>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    {connResult && (
                      <div className={`rounded-xl p-4 border text-xs space-y-2 ${connResult.ok ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800'}`}>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-bold">{connResult.message || (connResult.ok ? 'SuccÃƒÂ¨s' : 'Ãƒâ€°chec')}</span>
                        </div>
                        {devMode && !!connResult.raw && (
                          <div className="mt-2">
                            <p className="font-bold text-gray-500 uppercase text-[9px] mb-1">RÃƒÂ©ponse brute de l&apos;API (JSON)</p>
