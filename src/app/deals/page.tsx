    ...
    <>
      ...
      <section className="bg-gradient-to-br from-red-600 via-rose-600 to-pink-600 text-white py-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 bg-white/20 px-4 py-1.5 rounded-full text-sm font-bold mb-5">
            <Tag className="w-4 h-4" /> {t.deals.badge}
          </div>
          <h1 className="text-4xl sm:text-5xl font-black mb-3">{t.deals.title}</h1>
          ...
          <p className="text-red-200 text-sm">{discounted.length} {t.deals.available}</p>
        </div>
      </section>
      ...
      <div className="bg-white border-b border-gray-100 sticky top-16 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-6 overflow-x-auto">
          <span className="text-sm font-bold text-gray-900 whitespace-nowrap">
            🔥 {t.deals.dealsCount.replace('{n}', String(discounted.length))}
          </span>
          {byNiche.map(({ niche, products: ps }) => (
            ...
            >
              ...
              {(t.niches[niche.id as NicheKey]?.name ?? niche.name).split(' ')[0]} ({ps.length})
            </a>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-14">
        {byNiche.map(({ niche, products: nicheDeals }) => {
          const nicheName = t.niches[niche.id as NicheKey]?.name ?? niche.name
          return (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-gray-900">
                      {t.deals.nicheDeals.replace('{name}', nicheName)}
                    </h2>
                    <p className="text-sm text-gray-500">{nicheDeals.length} {t.deals.discountedProducts}</p>
                  </div>
                </div>
                <Link
                  href={`/${niche.id}`}
