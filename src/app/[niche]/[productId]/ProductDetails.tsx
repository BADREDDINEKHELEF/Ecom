  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-28 lg:pb-8">
      ...
      <nav className="text-sm text-gray-400 mb-8 flex items-center gap-2 flex-wrap">
        ...
        <Link href={`/${nicheId}`} className="hover:text-gray-700 transition-colors">{niche.name}</Link>
        <ArrowRight className="w-3 h-3" />
        <Link href={`/${nicheId}?category=${encodeURIComponent(product.category)}`} className="hover:text-gray-700 transition-colors">
          {product.category}
        </Link>
        <ArrowRight className="w-3 h-3" />
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="bg-gray-50 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">{t.product.deliveryTime}</p>
            </div>
          </div>
      ...
      {product.stock > 0 && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 px-4 py-3 flex gap-3 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
          ...
          <a
            href={`https://wa.me/213555000000?text=${encodeURIComponent(whatsappMessage)}`}
            target="_blank"
