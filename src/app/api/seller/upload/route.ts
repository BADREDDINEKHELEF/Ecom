...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Auth + vendor permission (supports team members with products:create/products:update)
    const authResult = await requireVendorPermission(req, 'products:create', response)
    if (authResult instanceof NextResponse) return authResult
    const { ctx } = authResult

    const userRl = await checkUserDualRateLimit(ctx.user.id, 'upload', {
    ...
    const ext   = file.type === 'image/webp' ? 'webp'
                ...
                : 'jpg'
    const path  = `vendors/${ctx.vendor.id}/${Date.now()}-${randomBytes(16).toString('hex')}.${ext}`
