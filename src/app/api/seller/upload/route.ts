...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (authErr || !user) {
      ...
      return copyCookies(response, NextResponse.json({ error: 'Non connecté. Reconnectez-vous.' }, { status: 401 }))
    }
    ...
    if (!vendorRow) {
      ...
      return copyCookies(response, NextResponse.json({ error: 'Compte vendeur introuvable.' }, { status: 403 }))
    }
    ...
    try {
      ...
      return copyCookies(response, NextResponse.json({ error: 'Invalid form data' }, { status: 400 }))
    }

    const file = formData.get('file')
    if (!(file instanceof File)) {
      return copyCookies(response, NextResponse.json({ error: 'No file provided' }, { status: 400 }))
    }
    ...
    if (uploadErr) {
      ...
      return copyCookies(response, NextResponse.json({ error: 'Échec de l\'upload. Réessayez.' }, { status: 500 }))
    }
    ...
    return copyCookies(response, NextResponse.json({ url: publicUrl }, { status: 201 }))
  } catch (err) {
    logger.error('[POST /api/seller/upload]', { error: err instanceof Error ? err.message : String(err) })
    return copyCookies(response, NextResponse.json({ error: 'Upload failed. Please try again.' }, { status: 500 }))
  }
}
