...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (!rate) {
        if (isLiveProvider) {
          logger.warn(`[delivery/rates] provider returned no rate — falling back to static pricing`, { provider, wilaya })
        }
        return NextResponse.json(staticRate(wilaya))
      }
      ...
      logger.warn(`[delivery/rates] provider threw — falling back to static pricing`, { provider, wilaya, error: err instanceof Error ? err.message : String(err) })
      return NextResponse.json(staticRate(wilaya))
    }
  } catch {
    return NextResponse.json(staticRate(wilaya))
