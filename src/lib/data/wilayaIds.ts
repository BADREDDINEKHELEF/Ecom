/**
 * Algerian wilaya name → numeric ID mapping.
 * Official DZ ordering (1–58). Shared across delivery providers that require
 * numeric IDs (ZR Express, Maystro, etc.) instead of wilaya name strings.
 */

export const WILAYA_NAME_TO_ID: Record<string, number> = {
...
export function wilayaNameToId(name: string): number | null {
  return WILAYA_NAME_TO_ID[name] ?? WILAYA_NAME_TO_ID[name.trim()] ?? null
}

export function wilayaNameToIdOrThrow(providerName: string, name: string): number {
  const id = wilayaNameToId(name)
  if (id === null) throw new Error(`${providerName}: unknown wilaya "${name}"`)
  return id
}
